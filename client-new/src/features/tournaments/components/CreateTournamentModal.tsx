...
import { useCreateTournament } from '../hooks/useTournamentRepository';
...
export const CreateTournamentButton: React.FC<CreateTournamentButtonProps> = ({ 
  onTournamentCreated 
}) => {
  const [modalVisible, setModalVisible] = useState(false);
  const [tournamentName, setTournamentName] = useState('');
  
  // Get the tournament creation function from the repository hook
  const createTournament = useCreateTournament();
  
  // Get the query client invalidation function
  const { useGetAll } = useTournamentQueries();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await createTournament(tournamentName.trim());
