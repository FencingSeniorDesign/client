...
This document outlines the step-by-step plan for migrating the current codebase to the new domain-based structure in `../client-new`.
...
1. **Phase 1: Copy Base Structure**
   - ✅ Create domain directory structure (already done)
   - ✅ Create core infrastructure files (already done)
...
4. **Phase 4: Split Monolithic Files**
   - Split `TournamentDatabaseUtils.ts` into domain repositories
   - Split `TournamentDataProvider.ts` into domain services
   - Split hook files into domain-specific hooks
...
This file needs to be broken down into domain repositories:

1. Identify database operations by domain
2. Extract domain-specific operations to relevant repository files
3. Use the base repository pattern from infrastructure
4. Keep domain-specific database logic with its feature

### Splitting TournamentDataProvider.ts

This provider handles state across domains:

1. Identify state management by domain
2. Extract domain-specific state to relevant service files
3. Create focused providers for each domain
4. Use React Context efficiently for cross-domain needs
