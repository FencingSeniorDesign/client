...
const TestComponent = () => {
    ...
    return (
        <>
            ...
            <Text testID="refresh" onPress={() => refreshAbility('Test Tournament')}>
                Refresh
            </Text>
            <Text testID="setContext" onPress={() => setTournamentContext('New Tournament')}>
                Set Context
            </Text>
        </>
    );
...
describe('AbilityContext', () => {
    ...
    it('should handle unknown role from server', async () => {
        ...
        await waitFor(() => {
            expect(console.warn).toHaveBeenCalledWith(
                expect.stringContaining(`AbilityProvider: Received unknown role string 'unknown_role'`)
            );
        });
    });
    ...
    it('should handle remote connections correctly', async () => {
        ...
        await waitFor(() => {
            expect(getByTestId('role').props.children).toBe(Role.VIEWER);
            expect(consoleLogSpy).toHaveBeenCalledWith('Remote connection detected.');
        });
    ...
    it('should handle errors in determineRole function', async () => {
        ...
        await waitFor(() => {
            expect(getByTestId('role').props.children).toBe(Role.VIEWER);
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error determining role:', expect.any(Error));
        });
    ...
    it('should skip refresh when tournament context is not changed', async () => {
        ...
        await waitFor(() => {
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Setting tournament context to:'));
        });
