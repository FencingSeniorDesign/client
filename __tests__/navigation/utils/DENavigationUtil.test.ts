...
describe('DENavigationUtil', () => {
  ...
  describe('navigateToDEPage', () => {
    ...
    it('handles invalid inputs gracefully', () => {
      ...
      navigateToDEPage(mockNavigation as any, undefined as any, { id: 1, type: 'de', deformat: 'single' } as any, 0);
      expect(mockNavigation.navigate).not.toHaveBeenCalled();
  ...
  describe('getDEFormatDescription', () => {
    it('returns correct description for each format', () => {
      ...
      expect(getDEFormatDescription('double')).toBe('Fencers continue in a losers bracket after their first loss.');
      expect(getDEFormatDescription('compass')).toBe('Fencers proceed through different brackets based on when they lose.');
