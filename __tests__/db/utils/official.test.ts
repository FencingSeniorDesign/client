...
describe('dbDeleteOfficial', () => {
    it('deletes an official and its event associations', async () => {
        ...
        expect(mockDeleteOfficialEventsWhere).toHaveBeenCalledWith(
            eq(schema.officialEvents.officialid, 99)
        );
        expect(mockDeleteOfficialsWhere).toHaveBeenCalledWith(
            eq(schema.officials.id, 99)
        );
    });
