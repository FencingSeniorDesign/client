...
export async function dbListOngoingTournaments(): Promise<Tournament[]> {
  try {
    const tournaments = await db.select()
      ...
      .where(eq(schema.tournaments.iscomplete, false));
...
export async function dbListCompletedTournaments(): Promise<Tournament[]> {
  try {
    const tournaments = await db.select()
      .from(schema.tournaments)
      .where(eq(schema.tournaments.iscomplete, true));
...
export async function dbAddFencerToEventById(fencer: Fencer, event: Event): Promise<void> {
  try {
    ...
    await db.insert(schema.fencerEvents)
      .values({
        fencerid: fencer.id,
        eventid: event.id
      })
...
export async function dbDeleteFencerFromEventById(fencer: Fencer, event: Event): Promise<void> {
  try {
    ...
    await db.delete(schema.fencerEvents)
      .where(
        and(
          eq(schema.fencerEvents.fencerid, fencer.id),
          eq(schema.fencerEvents.eventid, event.id)
        )
      );
  } catch (error) {
...
export async function dbGetPoolsForRound(roundId: number): Promise<{ poolid: number; fencers: Fencer[] }[]> {
  try {
    const results = await db
      ...
      .orderBy(
        schema.fencerPoolAssignment.poolid,
...
export async function dbGetBoutsForPool(roundId: number, poolId: number): Promise<Bout[]> {
  try {
    ...
    const poolFencers = await db.select({ count: count() })
      .from(schema.fencerPoolAssignment)
      .where(and(
        eq(schema.fencerPoolAssignment.roundid, roundId),
        eq(schema.fencerPoolAssignment.poolid, poolId)
      ));
    
    console.log(`Pool ${poolId} has ${poolFencers[0]?.count || 0} fencers assigned`);
    
    // Check if bouts exist for this pool
    const simpleBoutCount = await db.select({ count: count() })
      .from(schema.bouts)
    ...
    const bouts = await db
      ...
      .innerJoin(
        fpa1,
        and(
          eq(schema.bouts.lfencer, fpa1.fencerid),
          eq(schema.bouts.roundid, fpa1.roundid)
        )
      )
      .innerJoin(
        fpa2,
      ...
      .innerJoin(
        leftFencer,
        eq(schema.bouts.lfencer, leftFencer.id)
      )
      .innerJoin(
        rightFencer,
        eq(schema.bouts.rfencer, rightFencer.id)
      )
      .leftJoin(
        fb1,
        and(
          eq(fb1.boutid, schema.bouts.id),
          eq(fb1.fencerid, schema.bouts.lfencer)
        )
      )
      .leftJoin(
        fb2,
        and(
          eq(fb2.boutid, schema.bouts.id),
          eq(fb2.fencerid, schema.bouts.rfencer)
        )
      )
      .where(
        and(
          eq(schema.bouts.roundid, roundId),
          eq(fpa1.poolid, poolId),
          eq(fpa2.poolid, poolId)
        )
      );
    ...
    if (!bouts.length) {
      ...
      const simpleBouts = await db.select({
        id: schema.bouts.id,
        lfencer: schema.bouts.lfencer,
        rfencer: schema.bouts.rfencer,
        victor: schema.bouts.victor
  ...
  try {
    await db.update(schema.fencerBouts)
      ...
      .where(
        and(
          eq(schema.fencerBouts.boutid, boutId),
          eq(schema.fencerBouts.fencerid, fencerId)
        )
      );
  } catch (error) {
...
export async function dbGetSeedingForRound(roundId: number): Promise<any[]> {
  try {
    const results = await db
      ...
      .innerJoin(
        schema.fencers, 
        eq(schema.seedingFromRoundResults.fencerid, schema.fencers.id)
      )
      .where(eq(schema.seedingFromRoundResults.roundid, roundId))
...
export async function dbCreateReferee(referee: {
  ...
  try {
    const result = await db.insert(schema.referees)
      .values({
        fname: referee.fname,
        lname: referee.lname,
...
export async function dbAddRefereeToEvent(refereeId: number, eventId: number): Promise<void> {
  try {
    await db.insert(schema.refereeEvents)
      .values({
        refereeid: refereeId,
        eventid: eventId
      })
...
export async function dbGetRefereesForEvent(eventId: number): Promise<any[]> {
  try {
    const result = await db
      ...
      .from(schema.referees)
      .innerJoin(
        schema.refereeEvents, 
        eq(schema.referees.id, schema.refereeEvents.refereeid)
      )
...
export async function dbDeleteReferee(refereeId: number): Promise<void> {
  try {
    ...
    await db.delete(schema.refereeEvents)
      .where(eq(schema.refereeEvents.refereeid, refereeId));
    
    // Then, delete the referee record
    await db.delete(schema.referees)
...
export async function dbCreateOfficial(official: {
  ...
  try {
    const result = await db.insert(schema.officials)
      .values({
...
export async function dbAddOfficialToEvent(officialId: number, eventId: number): Promise<void> {
  try {
    await db.insert(schema.officialEvents)
      .values({
...
export async function dbGetOfficialsForEvent(eventId: number): Promise<any[]> {
  try {
    const result = await db
      ...
      .from(schema.officials)
      .innerJoin(
        schema.officialEvents, 
        eq(schema.officials.id, schema.officialEvents.officialid)
      )
...
export async function dbDeleteOfficial(officialId: number): Promise<void> {
  try {
    ...
    await db.delete(schema.officialEvents)
      .where(eq(schema.officialEvents.officialid, officialId));
...
export async function dbGetDEFormat(roundId: number): Promise<'single' | 'double' | 'compass'> {
  try {
    const rounds = await db.select({ deformat: schema.rounds.deformat })
      .from(schema.rounds)
      .where(eq(schema.rounds.id, roundId))
      .limit(1);
...
export async function dbInitializeRound(
  ...
  try {
    console.log(`Initializing round ${round.id} (order: ${round.rorder}) for event ${event.id}, type: ${round.type}, with ${fencers.length} fencers`);
    ...
    if (round[0].type === 'pool') {
      ...
      if (round[0].deformat === 'single') {
        ...
        const bouts = await db
          ...
          .from(schema.bouts)
          .innerJoin(
            schema.deBracketBouts, 
            eq(schema.bouts.id, schema.deBracketBouts.bout_id)
          )
          .leftJoin(
            leftFencer, 
            eq(schema.bouts.lfencer, leftFencer.id)
...
export async function dbGetCompassBracketBouts(
  ...
  try {
    ...
    const allBouts = await db.select({
      id: schema.bouts.id,
      lfencer: schema.bouts.lfencer,
      rfencer: schema.bouts.rfencer,
      victor: schema.bouts.victor,
      ...
      seed_right: rightSeeding.seed
        ...
        .leftJoin(leftFencerBout, and(
            eq(leftFencerBout.boutid, schema.bouts.id),
            eq(leftFencerBout.fencerid, schema.bouts.lfencer)
        ))
        .leftJoin(rightFencerBout, and(
            eq(rightFencerBout.boutid, schema.bouts.id),
            eq(rightFencerBout.fencerid, schema.bouts.rfencer)
        ))
        .leftJoin(leftSeeding, and(
            eq(leftSeeding.fencerid, schema.bouts.lfencer),
            eq(leftSeeding.roundid, schema.bouts.roundid)
        ))
        .leftJoin(rightSeeding, and(
            eq(rightSeeding.fencerid, schema.bouts.rfencer),
            eq(rightSeeding.roundid, schema.bouts.roundid)
        ))
        .where(eq(schema.bouts.roundid, roundId))
...
export async function dbIsDERoundComplete(roundId: number): Promise<boolean> {
  try {
    ...
    if (round[0].deformat === 'single') {
      ...
      const finalBoutResult = await db.select({
        count: count()
      })
          .from(schema.bouts)
          .where(
      ...
      const eastFinalResult = await db.select({
        count: count()
      })
          .from(schema.bouts)
          .innerJoin(
...
export async function dbGetDESummary(roundId: number): Promise<{
  ...
  try {
    ...
    const fencersResult = await db.select({
      count: count()
    })
    ...
    const currentRoundResult = await db.select({
    ...
    // Get the top 4 seeds
    const topSeeds = await db.select({
      seed: schema.seedingFromRoundResults.seed,
      id: schema.fencers.id,
      fname: schema.fencers.fname,
      lname: schema.fencers.lname,
  ...
  try {
    const result = await db.select({
      ...
      age: schema.events.age
        ...
        .innerJoin(
            schema.rounds,
            eq(schema.events.id, schema.rounds.eventid)
        )
        .where(eq(schema.rounds.id, roundId))
        .limit(1);
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (isBye && victor) {
        if (fencerA) {
          ...
          console.log(`Set score 15-0 for fencer ${fencerA.id} in bout ${boutId}`);
        } else if (fencerB) {
          await db.update(schema.fencerBouts)
              .set({ score: 15 })
              .where(
          ...
          console.log(`Set score 15-0 for fencer ${fencerB.id} in bout ${boutId}`);
        }
      }
    }
...
export async function dbCreateDoubleEliminationBracket(
  ...
  try {
    ...
    for (const bout of allBrackets) {
      ...
      await db.insert(schema.deBracketBouts)
          .values({
            roundid: roundId,
            bout_id: boutId,
...
export async function dbUpdateDEBoutAndAdvanceWinner(
  ...
  try {
    ...
    const boutsInSameTable = await db.select({
      id: schema.bouts.id,
      victor: schema.bouts.victor
    })
        .from(schema.bouts)
        .where(
            and(
                eq(schema.bouts.roundid, bout[0].roundid),
                eq(schema.bouts.tableof, bout[0].tableof)
            )
        );
