...
export const officialEvents = sqliteTable('OfficialEvents', {
  officialid: integer('officialid').notNull().references(() => officials.id),
...
export const refereeEvents = sqliteTable('RefereeEvents', {
  refereeid: integer('refereeid').notNull().references(() => referees.id),
  eventid: integer('eventid').notNull().references(() => events.id),
}, (table) => {
  return {
...
export const fencerEvents = sqliteTable('FencerEvents', {
  fencerid: integer('fencerid').notNull().references(() => fencers.id),
  eventid: integer('eventid').notNull().references(() => events.id),
}, (table) => {
  return {
...
export const fencerPoolAssignment = sqliteTable('FencerPoolAssignment', {
  roundid: integer('roundid').notNull().references(() => rounds.id),
