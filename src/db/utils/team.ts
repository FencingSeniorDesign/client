...
import { teams, teamMembers, teamBouts, teamBoutScores, relayBoutState, teamPoolAssignment, fencers, events } from '../schema';
...
export async function getEventTeams(client: DrizzleClient, eventid: number): Promise<TeamWithMembers[]> {
    const teamsResult = await client.select().from(teams)
...
export async function addTeamMember(client: DrizzleClient, member: TeamMemberCreate): Promise<number> {
    ...
    const existingMembership = await client.select()
        .from(teamMembers)
        .innerJoin(teams, eq(teamMembers.teamid, teams.id))
        .where(and(
            eq(teams.eventid, 
                sql`(SELECT eventid FROM ${teams} WHERE id = ${member.teamid})`
            ),
    ...
    if (member.role === 'starter' && member.position) {
        const existingPosition = await client.select()
            .from(teamMembers)
            .where(and(
                eq(teamMembers.teamid, member.teamid),
                eq(teamMembers.position, member.position)
            ));
...
export async function removeTeamMember(client: DrizzleClient, teamid: number, fencerid: number): Promise<void> {
    await client.delete(teamMembers).where(and(
        eq(teamMembers.teamid, teamid),
        eq(teamMembers.fencerid, fencerid)
    ));
}
...
export async function updateTeamMember(
    ...
    await client.update(teamMembers).set(updates).where(and(
        eq(teamMembers.teamid, teamid),
        eq(teamMembers.fencerid, fencerid)
    ));
}
...
export async function substituteTeamMember(
    ...
    const outMember = await client.select()
        .from(teamMembers)
        .where(and(
            eq(teamMembers.teamid, teamid),
            eq(teamMembers.fencerid, outFencerId),
            eq(teamMembers.role, 'starter')
        ));
    ...
    const inMember = await client.select()
        .from(teamMembers)
        .where(and(
            eq(teamMembers.teamid, teamid),
...
export async function seedTeams(client: DrizzleClient, eventid: number): Promise<void> {
    ...
    const teamsWithStrength = await Promise.all(eventTeams.map(async (team) => {
        const starters = team.members.filter(m => m.role === 'starter');
        let totalStrength = 0;
        
        for (const starter of starters) {
            const fencerData = await client.select()
                .from(fencers)
                .where(eq(fencers.id, starter.fencerid));
            
            if (fencerData.length > 0) {
                const fencer = fencerData[0];
                const eventData = await client.select()
                    .from(events)
...
export async function seedTeamsForEvent(
    client: DrizzleClient,
    eventid: number
): Promise<void> {
...
// Validation helpers
export async function validateTeamRoster(client: DrizzleClient, teamid: number): Promise<{ valid: boolean; errors: string[] }> {
