...
export async function getNCAABoutStatus(client: DrizzleClient, teamBoutId: number): Promise<NCAABoutStatus | null> {
    console.log(`Getting NCAA bout status for teamBoutId: ${teamBoutId}`);

    const [teamBout] = await client.select().from(teamBouts).where(eq(teamBouts.id, teamBoutId));

    console.log('Found team bout:', teamBout);

    if (!teamBout) {
        console.log('No team bout found');
        return null;
    }

    if (teamBout.format !== 'NCAA') {
        console.log(`Team bout format is '${teamBout.format}', not 'NCAA'`);
        return null;
    }

    // Get bout scores first
    let boutScoresData = await client
        .select()
        .from(teamBoutScores)
        .where(eq(teamBoutScores.team_bout_id, teamBoutId))
        .orderBy(teamBoutScores.bout_number);

    console.log(`Found ${boutScoresData.length} existing bout scores`);

    // If no bout scores exist and both teams are present, create them
    if (boutScoresData.length === 0 && teamBout.team_a_id && teamBout.team_b_id) {
        console.log('Creating missing NCAA bout scores...');
        try {
            // Get starters for both teams
            const teamAStarters = await getTeamStarters(client, teamBout.team_a_id);
            const teamBStarters = await getTeamStarters(client, teamBout.team_b_id);

            console.log(`Team A starters: ${teamAStarters.length}, Team B starters: ${teamBStarters.length}`);
            ...
            if (teamAStarters.length === 3 && teamBStarters.length === 3) {
                ...
                await client.insert(teamBoutScores).values(boutScoreInserts);
                console.log('Successfully created NCAA individual bout scores');

                // Re-query the bout scores
                boutScoresData = await client
                    .select()
                    .from(teamBoutScores)
                    .where(eq(teamBoutScores.team_bout_id, teamBoutId))
                    .orderBy(teamBoutScores.bout_number);

                console.log(`After creation, found ${boutScoresData.length} bout scores`);
            } else {
                console.log('Cannot create bout scores - teams do not have exactly 3 starters each');
            }
        } catch (error) {
            console.error('Error creating missing bout scores:', error);
        }
    }

    // Get fencer names separately to avoid complex joins
    const boutScores: NCAAIndividualBout[] = [];

    for (const boutScore of boutScoresData) {
        const [fencerA] = await client
            .select({
                id: fencers.id,
                fname: fencers.fname,
                lname: fencers.lname,
            ...
            .from(fencers)
            .where(eq(fencers.id, boutScore.fencer_a_id!));

        const [fencerB] = await client
            .select({
                id: fencers.id,
                fname: fencers.fname,
                lname: fencers.lname,
            ...
            .from(fencers)
            .where(eq(fencers.id, boutScore.fencer_b_id!));

        boutScores.push({
            boutNumber: boutScore.bout_number,
            fencerAId: boutScore.fencer_a_id!,
            fencerBId: boutScore.fencer_b_id!,
            fencerAName: fencerA ? `${fencerA.fname} ${fencerA.lname}` : 'Unknown',
            fencerBName: fencerB ? `${fencerB.fname} ${fencerB.lname}` : 'Unknown',
            fencerAScore: boutScore.fencer_a_score,
            fencerBScore: boutScore.fencer_b_score,
            winnerId: boutScore.winner_id,
            isComplete: boutScore.is_complete === 1,
        });
    }

    console.log(`Final boutScores array has ${boutScores.length} items`);
...
export async function updateNCAABoutScore(
    ...
    await client.transaction(async tx => {
        ...
        const [boutScore] = await tx
            .select()
            .from(teamBoutScores)
            .where(and(eq(teamBoutScores.team_bout_id, teamBoutId), eq(teamBoutScores.bout_number, boutNumber)));
        ...
        const isComplete =
            fencerAScore >= NCAA_TOUCHES_PER_BOUT ||
            fencerBScore >= NCAA_TOUCHES_PER_BOUT ||
...
export async function completeNCAAIndividualBout(
    ...
    await client.transaction(async tx => {
        ...
        if (boutScore.fencer_a_score < NCAA_TOUCHES_PER_BOUT && boutScore.fencer_b_score < NCAA_TOUCHES_PER_BOUT) {
            throw new Error('Cannot complete bout - neither fencer has reached 5 touches');
        }

        const winnerId =
            boutScore.fencer_a_score > boutScore.fencer_b_score ? boutScore.fencer_a_id : boutScore.fencer_b_id;
...
async function updateTeamScores(tx: any, teamBoutId: number): Promise<void> {
    ...
    const boutScores = await tx.select().from(teamBoutScores).where(eq(teamBoutScores.team_bout_id, teamBoutId));
    ...
    for (const bout of boutScores) {
        if (bout.is_complete && bout.winner_id) {
            ...
            const teamAMembers = await tx
                .select()
                .from(teamBoutScores)
                .where(
                    and(eq(teamBoutScores.team_bout_id, teamBoutId), eq(teamBoutScores.fencer_a_id, bout.winner_id))
                );
    ...
    const winnerId = isComplete ? (teamAWins > teamBWins ? teamBout.team_a_id : teamBout.team_b_id) : null;
    ...
    await client.transaction(async tx => {
        ...
        const futureBouts = await tx
            ...
            .where(
                and(
                    ...
                    or(eq(teamBoutScores.fencer_a_id, outFencerId), eq(teamBoutScores.fencer_b_id, outFencerId))
                )
            );
        ...
        for (const bout of futureBouts) {
            if (bout.fencer_a_id === outFencerId) {
                await tx.update(teamBoutScores).set({ fencer_a_id: inFencerId }).where(eq(teamBoutScores.id, bout.id));
            } else if (bout.fencer_b_id === outFencerId) {
                await tx.update(teamBoutScores).set({ fencer_b_id: inFencerId }).where(eq(teamBoutScores.id, bout.id));
            }
        }
    });
}
