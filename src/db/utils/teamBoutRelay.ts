...
export async function updateRelayBoutScore(
    ...
    await client.transaction(async (tx) => {
        ...
        const isComplete = teamAScore >= RELAY_TOTAL_TOUCHES || 
        ...
        if (currentLegNumber <= RELAY_MAX_LEGS && !isComplete) {
            ...
            if (currentLegNumber <= RELAY_BOUT_ORDER.length) {
                ...
                if (relayState.current_fencer_a_id !== expectedFencerA.fencerid || 
...
export async function updateRelayLegHistory(
    ...
    await client.update(relayLegHistory)
        .set({
            score_a: scoreA,
            score_b: scoreB,
...
export async function addRelayLegScore(
    ...
    await client.transaction(async (tx) => {
        ...
        if (nextLegNumber <= RELAY_MAX_LEGS && 
...
export async function recalculateRelayBoutFromHistory(
    client: DrizzleClient,
    teamBoutId: number
): Promise<void> {
    await client.transaction(async (tx) => {
