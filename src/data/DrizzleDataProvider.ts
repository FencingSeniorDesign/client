...
export class TournamentDataProvider {
    ...
    async getBoutsForPool(roundId: number, poolId: number): Promise<any[]> {
        console.log(
            `[DataProvider] Getting bouts for pool ${poolId} in round ${roundId}, remote: ${this.isRemoteConnection()}`
        );
     ...
     */
    async getPools(roundId: number): Promise<PoolData[]> {
        // Updated return type
    ...
    async updateBoutScore(boutId: number, scoreA: number, scoreB: number): Promise<boolean> {
        console.log(
            `[DataProvider] Updating bout ${boutId} score to ${scoreA}-${scoreB}, remote: ${this.isRemoteConnection()}`
        );
    ...
    async updatePoolBoutScores(
        ...
        console.log(
            `[DataProvider] Updating pool bout ${boutId} score to ${scoreA}-${scoreB}, winner: ${winnerId}, remote: ${this.isRemoteConnection()}`
        );

        if (this.isRemoteConnection()) {
            try {
                // Send update pool bout scores request to server with round and pool IDs for targeted cache invalidation
                tournamentClient.updatePoolBoutScores(
                    boutId,
                    scoreA,
                    scoreB,
                    fencerAId,
    ...
    async initializeRound(eventId: number, roundId: number): Promise<boolean> {
        console.log(
            `[DataProvider] Initializing round ${roundId} for event ${eventId}, remote: ${this.isRemoteConnection()}`
        );
    ...
    async getOfficials(tournamentName: string): Promise<Official[]> {
        console.log(
            `[DataProvider] Getting officials for tournament ${tournamentName}, remote: ${this.isRemoteConnection()}`
        );
    ...
    async getReferees(tournamentName: string): Promise<Official[]> {
        console.log(
            `[DataProvider] Getting referees for tournament ${tournamentName}, remote: ${this.isRemoteConnection()}`
        );
    ...
    async removeReferee(refereeId: number): Promise<boolean> {
        console.log(
            `[DataProvider] Removing referee ${refereeId} from tournament, remote: ${this.isRemoteConnection()}`
        );
    ...
    async removeOfficial(officialId: number): Promise<boolean> {
        console.log(
            `[DataProvider] Removing official ${officialId} from tournament, remote: ${this.isRemoteConnection()}`
        );
    ...
    async getTournamentRoleForDevice(deviceId: string, tournamentName: string): Promise<Role> {
        ...
        if (this.isRemoteConnection()) {
            console.warn(
                '[DataProvider] getTournamentRoleForDevice called on remote connection. Defaulting to VIEWER.'
            );
            return Role.VIEWER;
        }
        ...
        try {
            ...
            for (const event of events) {
                ...
                if (isOfficial) {
                    console.log(
                        `[DataProvider] Device ${deviceId} is an OFFICIAL for event ${eventId} in tournament ${tournamentName}`
                    );
                ...
                if (isReferee) {
                    console.log(
                        `[DataProvider] Device ${deviceId} is a REFEREE for event ${eventId} in tournament ${tournamentName}`
                    );
                    return Role.REFEREE;
                }
            }
